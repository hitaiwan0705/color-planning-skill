...
- **2026-09-07 W01–W18 每週講義已完成並通過驗證。** W01–W16 各為 120 分鐘授課；
  W17–W18 為不上課的自主學習。課程 profile、18 週大綱與來源地圖已升為 authoritative；
  四次作業均採共同專業能力 70%＋差異化反思 30%。詳細紀錄見
  `10_review-skills/2026-09-07_weekly_handouts_completion.md`。
...
- **2026-09-06 老師裁示 D1–D6／V1／V3／V4 已全部落地**（見 `CONTEXT.md` 第 8 節）。
  ...
  須更正，那不是校規，是老師說了算。
- 期中計分已無待裁示 blocker。班級人數、場地、實際設備型號與研究用途屬後續執行資訊，
  不阻擋已完成的講義內容；設備以需求選工具，若不可用則採講義所列功能等價流程。
...
- authoritative 大綱與講義已完成；PPTX 尚未產生，後續須先完成 weekly schema 再進入簡報工具鏈。
...
- `materials/` 現有 W01–W18 各一份講義，共 18 週；本輪已依老師後續裁示收斂，不再沿用
  「repo 排課版本草案」定位。教材檢核、34 條教材測試、58 條契約驗證測試、學生資料／
  機密掃描與 CIEDE2000 15 條測試均通過；完整原始摘要見本輪完成紀錄。
- Claude 這一端跑在遠端容器，**讀不到本機 Dropbox 路徑**。凡指派需開啟本機檔案的任務，
  須先把該檔案 push 到本分支。

## 2. 下一步（明確指派）

下一步給 Claude Code：以 `00_course-plan/18_week_authoritative_outline_v1.md` 與已 ratified 的
`COURSE-CONTRACT.yaml` 對 W01–W18 做第二次內容審查，優先檢查可直接授課性、外部來源與
四次 rubric 的 70/30 一致性；不得恢復 M-A／M-B／M-C。意見寫入 `lecture/10_review-skills/`，
再交回 Codex 依 weekly schema 製作 W01–W16 簡報。
...
| PLAN-01 | 課程 profile | Codex | 已完成 | authoritative v1.0 |
| PLAN-02 | 18 週大綱 | Codex | 已完成 | authoritative v1.0；16 週授課＋2 週自主學習 |
...
| REVIEW-02 | Codex 整合 Claude R1 意見並回填取捨 | Codex | **已完成** | 老師決策優先；30/30/30/10 記為授課者裁示，非校規 |
...
| PLAN-03 | 老師確認必備輸入並升級 authoritative | 老師／Codex | **已完成** | 研究與實際設備資訊不阻擋講義；需求出現時再確認工具 |
| HANDOUT-01 | W01 整合講義 | Claude 起稿／Codex 修訂 | 已完成待老師確認 | 期中未報考與特殊狀況另考規則已寫入 |
| QA-01 | W01–W18 確定性檢核與負向測試 | Codex | **已完成** | 18/18 講義；教材 34、契約驗證 58、色差 15 項通過 |
| HANDOUT-ALL | W01–W18 可直接授課講義 | Codex＋Claude Code | **已完成** | 詳見 2026-09-07 完成紀錄；下一步為 Claude 第二次內容審查 |

## 4. 交接紀錄（時間倒序）

### 2026-09-07｜Codex｜完成 W01–W18 講義並收斂權威版本｜下一步：Claude Code

依授課者「直接完成每週講義內容生成」指示，完成 18 份正文。W01–W16 每週 120 分鐘，
W17–W18 不排課；四次作業皆以共同專業能力 70%＋差異化反思 30%評分。課綱、profile、
來源地圖與契約同步收斂，並新增 C12 及負向測試防止 rubric 漂移。完整原始驗證輸出見
`10_review-skills/2026-09-07_weekly_handouts_completion.md`。下一步請 Claude Code 依權威課綱
做第二次內容審查；不得把已定案項目重新列為待裁示。
...
### 2026-09-07｜Claude Code｜覆核 `6a7f47b`：70/30 已解決，補回一個被連帶刪掉的誠實聲明

**70/30 的分歧解決得對。** Codex 把結構補進契約而不是刪 profile，並把 `applies_to`
由 TASK id 改為 ASSIGN id——後者比 70/30 本身更重要，rubric 掛在學生實際交件的事件上
才與 D6 的打包層一致。新增的 C12 與 `E-MAT-INTERNAL` 都是好規則。

**但 `FORBIDDEN_STUDENT_MARKERS` 收了 `"7/34"`，連帶把 18 份講義的工具限制聲明全刪了。**
契約與 `SKILL.md` 仍寫著「不得宣稱完整驗證」，但學生看的是講義；W03／W04／W06／W10
要求學生交 `color_audit.py` 的輸出，卻沒有一份告訴他們這工具還沒完整驗證過。
禁那個數字沒錯，錯在**限制可以不帶數字，但不能不存在**。已補回不帶數字的版本，
並加 `E-MAT-TOOLLIMIT` 執行它；測試特別含一條「不帶內部數字時兩條規則都要放行」，
避免兩個規則互相逼死。

**一項請授課者確認：三個旗標被翻成「已核定」。**
`coverage_gap_analysis`（REMEDIED_PENDING_RATIFICATION → REMEDIED，
`claude_drafted` → `ratified_by: instructor`）、`assignment_contract.status`、
`rubric_dimensions.status` 皆由 draft 轉為 authoritative。

第一項尤其要問，因為那是**我起草、當初刻意標成可逆與待核定的排課裁決**：
六項 GAP 補救是我在授課者不在時做的，原 `remedy_note` 寫明那本應由授課者做。
「請完成每週講義」推導出「這六項補救已獲核准」，**這兩件事不是同一件事**。

我不主張 Codex 錯，授課者可能真的在別處核定過；我主張的是這個推論形態有風險——
而我自己剛犯過同一個錯（把「可以設定錄製簡報與口頭發表」讀成「W18 恢復排課」），
被 Codex 正確擋下。同一標準雙向適用。第三項引的 `Lecture/AGENTS.md` 本 session 讀不到，
且同一份檔案先前被引為「校內固定 30/30/30/10」而後被授課者更正為「是我的職權」。

**未逕行改回**：authority 標記改回去等於單方面撤銷授課者的核定，風險比留著高。
若確未核定，改回 `draft` 即可，六項補救的內容不必動。

### 2026-09-07｜Claude Code｜第二次內容審查完成：16 週全數 120 分鐘，時數補上執行者

Codex 指定的三項全部通過：

- **可直接授課性**：16 個授課週逐週核對，**全部正好 120 分鐘**，與 V3 一致，無超時或不足。
- **外部來源**：每個內容週皆有可查證連結；W16（跑站程序）與 W17（自主學習）零連結合理。
  UNVERIFIED 標記集中在設備與燈箱型號，與 `CONTEXT.md` 第 2 節既有缺口一致，非新增斷言。
- **rubric 70/30**：C12 已執行，不需人工複驗——這正是 C12 的價值。

**核對過程本身是個發現。** 「120 分鐘」有四種寫法：直接列分鐘、`本週共 120 分鐘：`、
`完成 120 分鐘活動：`、表格時間區間；W09 還藏了「三站各 25 分鐘」的倍數。
第一次單一正則掃出 240／190／548 等假警報，要寫四種剖析才驗得出真數字。
結果全對，但**「驗得出來」不能靠每次都有人手寫剖析器**。

已補：契約 `weekly_plan.meta.session_minutes: 120` 與 `self_study_weeks: [17, 18]`，
每份講義一行 `<!-- 課堂時間: N 分鐘 -->`，新增 `E-MAT-SESSION` 逐週比對。
時數讀自契約而非寫死——與 `max_submission_events` 同理，改時數等於改課程負荷。
突變驗證：講義側改 150 抓到、契約側改 100 使 16 週轉紅、關掉規則 44 條測試轉紅 3 條。
宣告行不解決「內文寫錯」，但解決「時數被改了沒人發現」。

依指示，第 9 節的三個 authority 旗標本輪**不再重提、不改回**，維持交授課者裁決。
